    ...
    var support = {
        ...
        renderItem: function (isFile, data) {
            ...
            if (!isFile) {
                ...
                $item.append('<input type="hidden" class="picurl"></input>');
        ...
        deleteFile: function ($item, file) {
            ...
            var index = $item[0].children[4].defaultValue.lastIndexOf("/");  

            $item[0].children[4].defaultValue  = $item[0].children[4].defaultValue.substring(index + 1, $item[0].children[4].defaultValue.length);
            if($item[0].children[4].defaultValue == ''){
                if(that.getIsServerFile($item)) {  //服务端时
                    if(that.edit) {
                        that.deleteServerFileCore($item, that.deleteServerFileCallback);
                    } else {
                        that.$fns.log("can't delete server file");
                    }
                } else {
                    if(that.removeFileWithItem && file) {
                        that.removeFileWithItem(file);
                    }
                    that.loadChooseFileBtnStyle();
                }
            }else{
                swal({
                    title: "确定要删除这张图片吗？",
                    ...
                    if (yes) {
                        ...
                        $.ajax({
                            ...
                            data:{
                                filePath:$item[0].children[4].defaultValue,
                            },
